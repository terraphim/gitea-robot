#!/bin/bash
#
# integrate_codex_cli.sh - Configure Codex CLI to use gitea-robot MCP server
#
# This script detects the gitea-robot binary location and configures
# Codex CLI to use it as an MCP server for Gitea PageRank workflow.
#
# Usage: ./integrate_codex_cli.sh [gitea-robot-path]
#
...
# Function to escape strings for TOML
escape_toml() {
    echo "$1" | sed 's/"/\\"/g'
}
...
# Main script
echo "=========================================="
echo "Codex CLI - gitea-robot MCP Integration"
echo "=========================================="
echo
...
print_info "Configuring Codex CLI MCP server..."
...
if [ "$USE_1PASSWORD" = true ]; then
    ...
    WRAPPER_PATH="${CODEX_CONFIG_DIR}/gitea-robot-wrapper.sh"
    create_1password_wrapper "$ABSOLUTE_PATH" "$GITEA_URL" "$WRAPPER_PATH"
    print_success "Created 1Password wrapper script: $WRAPPER_PATH"
    
    # Create the MCP server configuration block (without env)
    MCP_CONFIG=$(cat << EOF

# Gitea Robot MCP Server - Added $(date '+%Y-%m-%d %H:%M:%S')
...
EOF
    ...
    # Create the MCP server configuration block (with plain text env)
    MCP_CONFIG=$(cat << EOF
...
# Check if config file exists and already has gitea_robot
if [ -f "$CODEX_CONFIG" ]; then
    print_info "Reading existing Codex CLI configuration..."
    
    # Backup existing config
    cp "$CODEX_CONFIG" "${CODEX_CONFIG}.backup.$(date +%Y%m%d_%H%M%S)"
    
    # Check if gitea_robot already exists in config
    if grep -q "\[mcp_servers.gitea_robot\]" "$CODEX_CONFIG"; then
        print_warning "Existing gitea_robot MCP server found in config"
        echo
        echo "Please manually update the configuration in: $CODEX_CONFIG"
        echo
        echo "Replace the existing [mcp_servers.gitea_robot] section with:"
        echo "$MCP_CONFIG"
        exit 1
    else
        # Append the new MCP server config
        echo "$MCP_CONFIG" >> "$CODEX_CONFIG"
        print_success "Added gitea-robot MCP server to existing config"
    fi
else
    print_info "Creating new Codex CLI configuration file..."
...
EOF
    ...
    print_success "Created new Codex CLI configuration"
fi

print_info "Configuration saved to: $CODEX_CONFIG"
...
echo "=========================================="
echo
echo "The gitea-robot MCP server has been configured for Codex CLI."
echo
...
echo "  1. Start a new Codex CLI session to load the MCP server"
...
echo "Note: Codex CLI caches MCP tools per session."
echo "      Start a new session if tools don't appear immediately."
echo

#!/bin/bash
#
# integrate_opencode.sh - Configure Opencode to use gitea-robot MCP server
#
# This script detects the gitea-robot binary location and configures
# Opencode to use it as an MCP server for Gitea PageRank workflow.
#
# Usage: ./integrate_opencode.sh [gitea-robot-path]
#
...
# Main script
echo "=========================================="
echo "Opencode - gitea-robot MCP Integration"
echo "=========================================="
echo
...
print_info "Configuring Opencode MCP server..."
...
if [ -f "$OPENCODE_CONFIG" ]; then
    print_info "Reading existing Opencode configuration..."
    CONFIG_CONTENT=$(cat "$OPENCODE_CONFIG")
else
    print_info "Creating new Opencode configuration file..."
    CONFIG_CONTENT='{"$schema": "https://opencode.ai/config.json"}'
fi
...
if command -v jq &> /dev/null; then
    ...
    if [ "$USE_1PASSWORD" = true ]; then
        ...
        echo "$CONFIG_CONTENT" | jq --arg path "$ABSOLUTE_PATH" \
            --arg url "$GITEA_URL" \
...
EOF
    ...
    if [ -f "$OPENCODE_CONFIG" ]; then
        print_error "Cannot merge with existing config without jq"
        echo "Please manually add the gitea-robot MCP server to: $OPENCODE_CONFIG"
        echo
        echo "Add the following to your mcp section:"
        cat "$TEMP_FILE"
        exit 1
    else
        mv "$TEMP_FILE" "$OPENCODE_CONFIG"
...
print_success "Opencode MCP configuration updated"
print_info "Configuration saved to: $OPENCODE_CONFIG"
...
echo "=========================================="
echo
echo "The gitea-robot MCP server has been configured for Opencode."
echo
...
echo "  1. Restart any running Opencode sessions to load the new MCP server"
echo "  2. Verify with: opencode mcp list"
echo "  3. The following tools will be available:"
